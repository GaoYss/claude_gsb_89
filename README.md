...
| 整改跟踪 | 整改状态机流转 + 整改流水（措施 / 进展 / 验收 / 销号），逾期自动标记；已销号隐患可申请重启 |
| 隐患重启 | 同类问题再次出现时基于原隐患单重新进入整改，保留原有流水，按轮次分开统计，历史月报不改写 |
...
首次启动会自动建表并灌入演示数据（7 座水库、12 条巡查记录、9 条隐患，其中 1 条为销号后重启在办）。已有数据时不会重复灌入；
...
已销号(closed)
  └─ 申请重启(需原因+依据+确认) ─> 待整改(registered)   # 新增第 2 轮整改任务，原流水保留
```
...
- 已销号隐患不能再追加整改记录，也不提供普通流转入口。

**已销号隐患重启**（`POST /api/v1/hazards/{id}/reopen`）：

- 仅「已销号」隐患可以申请重启；重启必须填写**重启原因**（同类问题再次出现的情况）、
  **重启依据**（巡查记录 / 现场复核 / 上级要求等）并勾选确认，否则返回 422。
- 重启**不新建隐患单**：隐患编号、描述与全部历史整改流水保留；系统新增一轮「整改任务」
  （`hazard_rectification_cycle`），状态回到「待整改」，销号日期清空，
  并写一条「重启整改」流水（含原因、依据、操作人）。
- **幂等**：隐患在办期间重复发起重启返回 409，同一条隐患的一次在办周期内只会产生一条新的
  整改任务；再次销号后才能发起下一轮（第 3 轮、第 4 轮……）。
- 重启后隐患在总览 / 列表 / 逾期统计中**不再计入已销号**，重新成为未销号隐患。
- 历史轮次的起止日期不可变：**办理时长、闭环率按轮次（首轮 / 重启后）分开统计**；
  月报按截至所选月末的快照计算（`GET /overview/rectification-monthly`），
  **已经出具的月报不会因后续重启而改写**。
...
| `hazard` | 隐患台账（编号唯一，含状态、等级、期限、累计重启次数） | N:1 水库；N:1 来源巡查（可空） |
| `hazard_rectification_cycle` | 整改轮次 / 整改任务（首轮 + 每次重启一轮，独立起止日期、重启原因与依据） | N:1 隐患 |
...
| GET | `/overview/summary` | 总览统计、待办隐患、最近巡查（含首轮 / 重启后闭环统计） |
| GET | `/overview/rectification-monthly` | 整改月报快照（year/month），历史月份不受重启影响 |
...
| POST | `/hazards/{id}/reopen` | 已销号隐患申请重启（原因 + 依据 + 确认） |
...
```bash
cd backend
python -m pytest            # 接口用例：台账 CRUD、编号生成、巡查结论推导、状态机、重启、逾期、统计
```
...
- 建表方式为启动时 `create_all`，未引入迁移框架；表结构变更后需要重建库或自行引入 Alembic。
  （隐患重启功能新增了 `hazard_rectification_cycle` 表与 `hazard.reopen_count`、
  `hazard_rectification.cycle_seq` 字段，旧库需重建或手工补建。）
